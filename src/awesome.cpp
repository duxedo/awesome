...
extern "C" void awesome_atexit(bool restart)
{
    ...
    xcb_change_property(getGlobals().connection, XCB_PROP_MODE_REPLACE,
                        ...
                        ATOM_AWESOME_CLIENT_ORDER, XCB_ATOM_WINDOW, 32, n, wins);
...
acquire_WM_Sn(bool replace)
{
    ...
    xcb_create_window(getGlobals().connection, getGlobals().screen->root_depth,
                      getGlobals().selection_owner_window, getGlobals().screen->root,
                      -1, -1, 1, 1, 0,
                      XCB_COPY_FROM_PARENT, getGlobals().screen->root_visual,
                      0, NULL);
    ...
    atom_q = xcb_intern_atom_unchecked(getGlobals().connection, false,
    ...
    get_sel_reply = xcb_get_selection_owner_reply(getGlobals().connection,
    ...
    if (get_sel_reply->owner != XCB_NONE)
    {
        ...
        do {
            p_delete(&geom_reply);
            geom_reply = xcb_get_geometry_reply(getGlobals().connection,
...
main(int argc, char **argv)
{
    ...
    getGlobals().default_depth = draw_visual_depth(getGlobals().screen, getGlobals().visual->visual_id);
    ...
    if(getGlobals().default_depth != getGlobals().screen->root_depth)
    {
        ...
        xcb_create_colormap(getGlobals().connection, XCB_COLORMAP_ALLOC_NONE,
                getGlobals().default_cmap, getGlobals().screen->root,
    ...
    if (xcb_cursor_context_new(getGlobals().connection, getGlobals().screen, &getGlobals().cursor_ctx) < 0)
...

    {
        ...
        cookie = xcb_change_window_attributes_checked(getGlobals().connection,
                                                      getGlobals().screen->root,
    ...
    if (getGlobals().have_shape)
    {
        xcb_shape_query_version_reply_t *reply =
            xcb_shape_query_version_reply(getGlobals().connection,
                    ...
                    NULL);
        getGlobals().have_input_shape = reply && (reply->major_version > 1 ||
