...
static void drawable_unset_surface(drawable_t* d) {
    ...
    if (d->pixmap) {
        xcb_free_pixmap(Manager::get().x.connection, d->pixmap);
    }
    d->refreshed = false;
    d->surface = NULL;
    d->pixmap = XCB_NONE;
...
void drawable_set_geometry(lua_State* L, int didx, area_t geom) {
    ...
    if (area_changed && geom.width > 0 && geom.height > 0) {
        ...
        d->surface = cairo_xcb_surface_create(
          Manager::get().x.connection, d->pixmap, Manager::get().visual, geom.width, geom.height);
...
void drawable_class_setup(lua_State* L) {
    constexpr auto methods = DefineClassMethods<&drawable_class>();
