...
event_handle_xfixes_selection_notify(xcb_generic_event_t *ev)
{
    ...
    while (lua_next(L, -2) != 0) {
        if (lua_type(L, -1) == LUA_TUSERDATA) {
            selection_watcher_t *selection = reinterpret_cast<selection_watcher_t*>(lua_touserdata(L, -1));
...
luaA_selection_watcher_set_active(lua_State *L, selection_watcher_t *selection)
{
    ...
    if(b != is_active)
    {
        if (b)
        {
            ...
            {
                xcb_xfixes_select_selection_input(getGlobals().connection, selection->window, selection->selection,
            ...
            if (getGlobals().have_xfixes)
                xcb_xfixes_select_selection_input(getGlobals().connection, selection->window, selection->selection, 0);
