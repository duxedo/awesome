...
drawin_t* drawin_allocator(lua_State* L) {
    ...
    const uint32_t values[] = {
      ...
      xcursor_new(&getGlobals().x.cursor_cache, getGlobals().x.cursor_ctx, w->cursor.c_str())};
...
static int luaA_drawin_set_cursor(lua_State *L, drawin_t *drawin) {
    ...
    if (buf) {
        xcb_cursor_t cursor = xcursor_new(&getGlobals().x.cursor_cache, getGlobals().x.cursor_ctx, buf->data());
        if (cursor) {
            drawin->cursor = *buf;
