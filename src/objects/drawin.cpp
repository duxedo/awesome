...
drawin_t* drawin_allocator(lua_State* L) {
    ...
    const uint32_t values[] = {
      ...
      xcursor_new(&Manager::get().x.cursor_cache, Manager::get().x.cursor_ctx, w->cursor.c_str())};
...
static int luaA_drawin_set_cursor(lua_State *L, drawin_t *drawin) {
    ...
    if (buf) {
        xcb_cursor_t cursor = xcursor_new(&Manager::get().x.cursor_cache, Manager::get().x.cursor_ctx, buf->data());
        if (cursor) {
            drawin->cursor = *buf;
...
void drawin_class_setup(lua_State* L) {
    ...
    constexpr auto methods = DefineClassMethods<&drawin_class>({
      {   "get", luaA_drawin_get},
