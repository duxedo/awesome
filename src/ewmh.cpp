...
ewmh_update_net_active_window(lua_State *L)
{
    ...
    getConnection().replace_property(getGlobals().screen->root, _NET_ACTIVE_WINDOW, XCB_ATOM_WINDOW, win);
    return 0;
}
...
ewmh_init(void)
{
    ...
    getConnection().replace_property(xscreen->root, _NET_SUPPORTING_WM_CHECK, XCB_ATOM_WINDOW, father);
...
ewmh_update_net_current_desktop(lua_State *L)
{
    ...
    getConnection().replace_property(getGlobals().screen->root, _NET_CURRENT_DESKTOP, XCB_ATOM_CARDINAL, idx);
    return 0;
}
...
ewmh_update_net_desktop_names(void)
{
    ...
    getConnection().replace_property(getGlobals().screen->root, _NET_DESKTOP_NAMES, UTF8_STRING, buf);
}
...
ewmh_client_check_hints(client_t *c)
{
    ...
    c0 = xcb_get_property_unchecked(getGlobals().connection, false, c->window,
                                    _NET_WM_DESKTOP, XCB_GET_PROPERTY_TYPE_ANY, 0, 1);

    c1 = xcb_get_property_unchecked(getGlobals().connection, false, c->window,
                                    _NET_WM_STATE, XCB_ATOM_ATOM, 0, UINT32_MAX);

    c2 = xcb_get_property_unchecked(getGlobals().connection, false, c->window,
...
ewmh_process_client_strut(client_t *c)
{
    ...
    xcb_get_property_cookie_t strut_q = xcb_get_property_unchecked(getGlobals().connection, false, c->window,
...
{
  return xcb_get_property_unchecked(getGlobals().connection, false, w,
