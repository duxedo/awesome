...
#define xwindow_set_name_static(win, name) \
      getConnection().icccm_set_wm_name(win, XCB_ATOM_STRING, 8, sizeof(name) - 1, name)
...
#define _xwindow_set_class_instance_static(win, instance_class) \
    Manager::get().x.connection.icccm_set_wm_class(win, sizeof(instance_class), instance_class)
